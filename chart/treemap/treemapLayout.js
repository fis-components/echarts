...
function update(ecModel, api, payload) {
    ...
    ecModel.eachComponent(condition, function (seriesModel) {
        ...
        if (payloadType !== 'treemapMove') {
            var rootSize = payloadType === 'treemapZoomToNode' ? estimateRootSize(seriesModel, targetInfo, containerWidth, containerHeight) : rootRect ? [
                rootRect.width,
                rootRect.height
            ] : [
                containerWidth,
...
function squarify(node, options) {
    ...
    for (var i = 0, len = viewChildren.length; i < len; i++) {
        var childOption = zrUtil.extend({ hideChildren: hideChildren }, options);
