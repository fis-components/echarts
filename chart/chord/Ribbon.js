...
module.exports = require('../../util/graphic').extendShape({
    ...
    buildPath: function (ctx, shape) {
        ...
        ctx.bezierCurveTo((cx - sx1) * 0.7 + sx1, (cy - sy1) * 0.7 + sy1, (cx - tx0) * 0.7 + tx0, (cy - ty0) * 0.7 + ty0, tx0, ty0);
        // Chord to self
        if (shape.s0 === shape.t0 && shape.s1 === shape.t1) {
            return;
        }
        ctx.arc(cx, cy, shape.r, t0, t1, !clockwise);
        ctx.bezierCurveTo((cx - tx1) * 0.7 + tx1, (cy - ty1) * 0.7 + ty1, (cx - sx0) * 0.7 + sx0, (cy - sy0) * 0.7 + sy0, sx0, sy0);
    }
}) || module.exports;;