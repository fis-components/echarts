...
module.exports = require('../../echarts').extendChartView({
    ...
    render: function (seriesModel, ecModel, api) {
        ...
        var polygon = this._polygon || (this._polygon = new graphic.Polygon({ shape: { points: [] } }));
        ...
        polyline.setStyle(zrUtil.extend(seriesModel.getModel('lineStyle.normal').getLineStyle(), { stroke: data.getVisual('color') }));
        var areaStyleModel = seriesModel.getModel('areaStyle.normal');
        polygon.ignore = areaStyleModel.isEmpty();
        graphic.setHoverStyle(polyline, seriesModel.getModel('lineStyle.emphasis').getLineStyle());
        if (!polygon.ignore) {
            polygon.setStyle(zrUtil.defaults(areaStyleModel.getAreaStyle(), {
                fill: data.getVisual('color'),
                opacity: 0.7
            }));
            graphic.setHoverStyle(polygon, seriesModel.getModel('areaStyle.emphasis').getLineStyle());
        }
        group.add(polyline);
        group.add(polygon);
