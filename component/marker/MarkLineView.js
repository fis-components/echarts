...
var markLineTransform = function (data, coordSys, baseAxis, valueAxis, precision, item) {
    ...
    if (!zrUtil.isArray(item) && mlType === 'min' || mlType === 'max' || mlType === 'average') {
        ...
        var value = mlType === 'average' ? data.getSum(valueAxis.dim, true) / data.count() : data.getDataExtent(valueAxis.dim)[mlType === 'max' ? 1 : 0];
...
function createList(coordSys, seriesData, mlModel) {
    ...
    if (coordSys) {
        ...
        var optData = zrUtil.filter(zrUtil.map(mlModel.get('data'), zrUtil.curry(markLineTransform, seriesData, coordSys, baseAxis, valueAxis, precision)), zrUtil.curry(markLineFilter, coordSys));
