...
require('../../echarts').extendComponentView({
    ...
    _showPolarPointer: function (axisPointerModel, polar, axisType, point) {
        ...
        function movePolarShadow(axisType, point, otherExtent) {
            ...
            if (axisType === 'angle') {
                targetShape = makeSectorShape(polar.cx, polar.cy, otherExtent[0], otherExtent[1], // In ECharts y is negative if angle is positive
    ...
    _dispatchAndShowSeriesTooltipContent: function (coordSys, seriesList, point, value, contentNotChange) {
        ...
        var payloadBatch = zrUtil.map(seriesList, function (series) {
            return {
                ...
                dataIndex: series.getAxisTooltipDataIndex ? series.getAxisTooltipDataIndex(series.getDimensionsOnAxis(baseAxis.dim), value, baseAxis) : series.getData().indexOfNearest(series.getDimensionsOnAxis(baseAxis.dim), value[baseAxis.dim === 'x' || baseAxis.dim === 'radius' ? 0 : 1])
            };
        });
        var lastHover = this._lastHover;
        var api = this._api;
